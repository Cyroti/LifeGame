...
//     // 用作get越界时的返回值？
//     None,
// } GridState;//这里应该是前人的一种设想，但我们很快看到，它被否决了

typedef int GridState; // 这里可以看到用了GridState来代替int
...
typedef struct
{
    ...
    GridState **_grids; //_grids就是用来表示个点状态的数字

    // 其它状态，可扩展...
    // float density;
} Conway; // 这里构建了Conway这种结构体

// 前言：涉及指针操作和内存分配，随时记得指针判空//谨慎的处理内存

// 可以看出下面全是函数原型，并没有函数的定义

// 构造新格点，分配内存
// 创建失败则返回NULL
// malloc()
Conway new_conway(const uint16_t m, const uint16_t n); // 这里的uint16_t即是stdint.h中的宏；等价于unsigned short，即其别名
// 删除格点，回收内存
// free()
